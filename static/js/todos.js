      ...
      <div class="card-body">
        ...
        <div class="d-flex justify-content-between align-items-start">
          ...
          <div>
            ...
            ${todo.description
              ? `<p class="card-text text-muted small mb-1">${escapeHtml(todo.description)}</p>`
              : ''}
...
function renderButtons(todo) {
  ...
  if (todo.status === 'active') {
    ...
    buttons.push(
      `<button class="btn btn-sm btn-outline-secondary" onclick="openEdit(${todo.id})">Изменить</button>`
    );

    buttons.push(
      `<button class="btn btn-sm btn-success" onclick="changeStatus(${todo.id}, 'done')">Выполнить</button>`
    );
  }
  ...
  if (todo.status === 'done') {
    ...
    buttons.push(
      `<button class="btn btn-sm btn-outline-secondary" onclick="changeStatus(${todo.id}, 'archived')">Архивировать</button>`
    );
  }
...
document.getElementById('modal-submit').addEventListener('click', async () => {
  ...
  if (id) {
    ...
    document.querySelectorAll('#statusTabs .nav-link')
      .forEach(b => b.classList.toggle('active', b.dataset.status === 'active'));
  }
